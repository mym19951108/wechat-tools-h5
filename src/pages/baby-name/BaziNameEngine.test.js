...
describe('generateBaziNames', () => {
  it('generates names with shared poetry from same poem', () => {
    ...
    result.forEach(r => {
      ...
      // Both chars should appear in the poetry text
  ...
  it('all names have both chars in poetry text', () => {
    const result = generateBaziNames({ surname: '陈', gender: 'any', xiShen: ['水', '木', '金', '火', '土'], jiShen: [] })
    expect(result.length).toBeGreaterThan(0)
    result.forEach(r => {
      expect(r.poetry).toBeTruthy()
      expect(r.poetry.text.includes(r.char1) && r.poetry.text.includes(r.char2)).toBe(true)
    })
  })
