// Build name entries from Chinese idioms (成语)
// Usage: node scripts/build-chengyu.js
import fs from 'fs'
...
// Load chengyu data
const moodPath = path.join(SCRIPTS_DIR, 'mood-heuristic.json');
const moodScores = JSON.parse(fs.readFileSync(moodPath, 'utf8'));
const idiomPath = path.join(SCRIPTS_DIR, 'idiom.json')
const idioms = JSON.parse(fs.readFileSync(idiomPath, 'utf8'))
console.log(`Loaded ${idioms.length} idioms`)

// Wuxing mapping (reuse from build-poetry-db.js)
const MANUAL_WUXING = {
...
// Filter: only positive chengyu with both chars having wuxing
const results = []
const seen = new Set()
...
for (const item of idioms) {
  ...
  // Extract 2-char pairs (0-1, 1-2, 2-3 for 4-char chengyu)
  ...
  for (let i = 0; i <= word.length - 2; i++) {
    ...
    results.push({
      ...
      annotation: explanation,
              mood: moodScores[pair] || 5
    })
  }
}
console.log(`Extracted ${results.length} name pairs from chengyu`)

// Group by wuxing
const groups = {}
for (const r of results) {
  if (!groups[r.wuxing]) groups[r.wuxing] = []
  groups[r.wuxing].push({ chars: r.chars, char1: r.char1, char2: r.char2, text: r.text, poem: r.poem, source: r.source, coherent: r.coherent, annotation: r.annotation, mood: r.mood })
}

// Write
const content = `// Generated from chinese-xinhua idiom database (30K+ chengyu)
// Auto-annotated: each entry's annotation comes from the chengyu's standard explanation
export const chengyuNames = ${JSON.stringify(groups, null, 2)}
`
fs.writeFileSync(path.join(OUTPUT_DIR, 'chengyu-names.js'), content)
console.log(`Written: src/data/chengyu-names.js (${(content.length / 1024).toFixed(1)} KB)`)
console.log('=== Done ===')
